    ...
    try {
      ...
      if (result.result && result.result.content) {
        ...
        palette.inks.forEach((ink, index) => {
          ...
          console.log(`     RGB: [${ink.ink.rgb.join(', ')}] | Distance: ${ink.distance?.toFixed(1) || 'N/A'}`);
        });
      } else {
...
runMCPTests().then(() => {
  ...
  console.log('✅ New predefined themes (summer, winter, pastel, vibrant, monochrome, sunset, forest)');
  console.log('✅ Harmony-based palette generation (complementary, analogous, triadic, split-complementary)');
