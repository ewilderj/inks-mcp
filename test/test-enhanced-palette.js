...
function testGetColorPalette(theme, paletteSize = 5, harmony = null) {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(paletteSize, targetColors.length); i++) {
      ...
      const closestInks = findClosestInks(targetRgb, inkColors, 5).filter(
      ...
      if (closestInks.length > 0) {
        ...
        console.log(
          `      Target: RGB(${targetRgb.join(', ')}) → Actual: RGB(${ink.rgb.join(', ')}) | Distance: ${ink.distance?.toFixed(1)}`,
        );
      }
    }
...
console.log(
  `Conversion accurate: ${JSON.stringify(testColor) === JSON.stringify(backToRgb) ? '✅' : '❌'}\n`,
);
...
['complementary', 'analogous', 'triadic', 'split-complementary'].forEach((harmony) => {
  ...
  harmonyColors.forEach((hsl, i) => {
    ...
    console.log(
      `  ${i + 1}. HSL(${hsl.map((v) => Math.round(v)).join(', ')}) → RGB(${rgb.join(', ')})`,
    );
  });
});
